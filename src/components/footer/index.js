...
import { FooterWrapper, FooterItem, FooterFont } from "./style";
...
class Footer extends PureComponent {
          ...
          return (
            <Link key={index} style={{ textDecoration: "none" }} to={item.link}>
              <FooterItem onClick={this.handleClick.bind(this, index)}>
                <span
                  className={item.active ? "iconfont iconFocus" : "iconfont"}
                  dangerouslySetInnerHTML={{ __html: item.iconId }}
                ></span>
                <FooterFont className={item.active ? "redFont" : ""}>
                  {item.title}
                </FooterFont>
              </FooterItem>
            </Link>
  ...
  handleClick(index) {
    const updatedTabList = JSON.parse(JSON.stringify(this.state.tabList));
